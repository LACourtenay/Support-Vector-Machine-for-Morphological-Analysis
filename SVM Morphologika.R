# Geometric Morphometric and Machine Learning Code
#
# Code written by:
#
...
# For scores
GPAdata<-procGPA(a$coords) # Generalised Procrustes fit in Shape Space

# For pits
GPAdata<-procGPA(a$coords, scale = FALSE) #Generalised Procrustes fit in Form Space

# Extract PC scores to train classification model---------------------------------------------------------------------------------------

pcscores<-GPAdata$scores # Extract PC Scores
svm_data<-data.frame(Sample = a$labels, pcscores) # Save as new data frame with labels
...
for (i in 50) { # Run the tuning process for 50 iterations
  svm_tune<-svm(Sample~., data = trainset, kernel = "radial", # SVM model using a radial kernel
                cost = runif(1, min = 0, max = 500), # run a random number per iteration with cost values between 0 and 500
                gamma = runif(1, min = 0, max = 500), # run a random number per iteration with gamma values between 0 and 500
